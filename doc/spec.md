...
Lw is a general purpose, statically typed, strict, impure, functional-first programming language that supports cutting-edge features and advanced forms of polymorphism for writing robust, reusable, maintenable and succinct code.

Ideal for writing big as well as small programs, since every feature has a lightweight counterpart, it integrates state-of-the-art advancements in the programming language field together with a number of novel bits invented or reinterpreted by myself throughout more than 15 years of passion, research and work.

Among the most highlights: type inference, System-F types and first-class polymorphism, open-world context-independent overloading with automatic as well as semi-automatic resolution, constrained parametric polymorphism, implicit parameters, fully-inferred union types and Generalized Algebraic Datatypes supporting overloadable data constructors, first-class modules, row types for polymorphic variants and lightweight records with overloadable labels supporting structual subtype polymorphism, powerful kind system supporting full inference, higher-order polymorphism, kind polymorphism, value-to-type & type-to-kind promotion and viceversa. And much more.

But what makes Lw unique is the way these features are tied together, unleashing some novel ways of writing and reusing code. For example, type constraints resolution is central to many language mechanisms, leading to a form of static dispatching that can either be automatic or controlled by the user; dually, row-typed records are subject to dynamic dispatching and enables structural subtyping. And here lies the magic: users can turn type constraints into records and viceversa anytime by using a special operator, converting non-first-class static entities into first-class types and values and the other way round. This makes two worlds communicate: the world of static resolution and the world of dynamic resolution. Languages typically do not define a clear symmetry in this respect, and anyway a lot of boilterplate code is often required for switching between the two worlds - when possible at all. In Lw this symmetry is crucial and explitly designed, encouraging code reuse even across orthogonal features.
